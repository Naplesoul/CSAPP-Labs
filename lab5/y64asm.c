...
int find(const char *begin, char target)
{
    int pos = 0;
    while(TRUE)
    {
...
int add_symbol(char *name)
{
    ...
    memset(new, 0, sizeof(symbol_t));

    new->name = (char*)malloc(MAX_INSLEN);
    memset(new->name, 0, MAX_INSLEN);
...
*           call SUM  #invoke SUM function */
    ...
    char *lineStart = line->y64asm;
    SKIP_BLANK(lineStart);
    char *linePtr = lineStart;
    
    // is line end ?
    /* is a comment ? */
    if(IS_END(linePtr) || IS_COMMENT(linePtr))
    {
        line->type = TYPE_COMM;
        return TYPE_COMM;
    }
    ...
    {
        char *name;
        if(parse_label(&linePtr, &name) == PARSE_ERR)
        {
            line->type = TYPE_ERR;
            return TYPE_ERR;
        }
    ...
    /* is an instruction ? */
    instr_t *inst;
    if(parse_instr(&linePtr, &inst) == PARSE_INSTR)
    {
        itype_t icode = HIGH(inst->code);
        // itype_t ifun = LOW(inst->code);
        regid_t regA = REG_NONE, regB = REG_NONE;
        int64_t immed = 0;
        int64_t *immedPos = NULL;
        char* sym = NULL;
        parse_t parseResult = PARSE_ERR;
        switch (icode)
        {
        case I_HALT:
        case I_NOP:
            break;
        case I_RRMOVQ:
            if(parse_reg(&linePtr, &regA) != PARSE_REG
            || parse_delim(&linePtr, ',') != PARSE_DELIM
            || parse_reg(&linePtr, &regB) != PARSE_REG)
            {
                err_print("Invalid ','");
                line->type = TYPE_ERR;
                return TYPE_ERR;
            ...
            break;
        case I_IRMOVQ:
            parseResult = parse_imm(&linePtr, &sym, &immed);
            if(parseResult == PARSE_ERR)
            {
                line->type = TYPE_ERR;
                return TYPE_ERR;
            }
            if(parseResult == PARSE_DIGIT)
            {
                immedPos = (int64_t*)((line->y64bin).codes + 2);
                *immedPos = immed;
            }
            else if(parseResult == PARSE_SYMBOL)
                add_reloc(sym, &(line->y64bin));

            if(parse_delim(&linePtr, ',') != PARSE_DELIM
            || parse_reg(&linePtr, &regB) != PARSE_REG)
            {
                err_print("Invalid ','");
                line->type = TYPE_ERR;
        ...
        case I_RMMOVQ:
            if(parse_reg(&linePtr, &regA) != PARSE_REG
            || parse_delim(&linePtr, ',') != PARSE_DELIM
            || parse_mem(&linePtr, &immed, &regB) != PARSE_MEM)
            {
                line->type = TYPE_ERR;
                return TYPE_ERR;
            }
            ...
            *immedPos = immed;
            ...
            break;
        case I_MRMOVQ:
            if(parse_mem(&linePtr, &immed, &regB) != PARSE_MEM
            || parse_delim(&linePtr, ',') != PARSE_DELIM
            || parse_reg(&linePtr, &regA) != PARSE_REG)
            {
                line->type = TYPE_ERR;
                return TYPE_ERR;
            }
        ...
        case I_JMP:
        case I_CALL:
            parseResult = parse_data(&linePtr, &sym, &immed);
            if(parseResult == PARSE_ERR)
            {
                line->type = TYPE_ERR;
                return TYPE_ERR;
            ...
            if(parseResult == PARSE_DIGIT)
            {
                ...
                {
                    maxDest = immed;
                    maxDestLineNo = lineno;
                }
                immedPos = (int64_t*)((line->y64bin).codes + 1);
                *immedPos = immed;
            }
            else if(parseResult == PARSE_SYMBOL)
                add_reloc(sym, &(line->y64bin));
        ...
        case I_DIRECTIVE:
            ...
            case D_DATA:
                ...
                if(parseResult == PARSE_DIGIT)
                {
                    immedPos = (int64_t*)((line->y64bin).codes);
                    *immedPos = immed;
                }
                else if(parseResult == PARSE_SYMBOL)
                    add_reloc(sym, &(line->y64bin));
                break;
            case D_POS:
                if(parse_digit(&linePtr, &immed) == PARSE_ERR)
                {
                    line->type = TYPE_ERR;
                    return TYPE_ERR;
                }
                ...
                break;
            case D_ALIGN:
                if(parse_digit(&linePtr, &immed) != PARSE_DIGIT)
                {
                    line->type = TYPE_ERR;
                    return TYPE_ERR;
                }
                if(vmaddr % immed == 0)
                    break;
                vmaddr = ((int64_t)(vmaddr/immed) + 1)*immed;
                break;
            default:
                return TYPE_ERR;
            }
...
int relocate(void)
{
    ...
    while (rtmp)
    {
        ...
        icode = HIGH((rtmp->y64bin->codes)[0]);
        switch(icode)
        {
        case I_IRMOVQ:
        case I_RMMOVQ:
        case I_MRMOVQ:
            immedPos = (int64_t*)((byte_t*)(rtmp->y64bin->codes) + 2);
            *immedPos = symb->addr;
            break;
        case I_JMP:
        case I_CALL:
            immedPos = (int64_t*)((byte_t*)(rtmp->y64bin->codes) + 1);
            *immedPos = symb->addr;
            break;
        // It is actually I_DIRECTIVE, 
        // in parse_line(), I_DIRECTIVE case will not change codes[0]
        case I_HALT:
            immedPos = (int64_t*)((byte_t*)(rtmp->y64bin->codes));
            *immedPos = symb->addr;
            break;
        default:
            return -1;
...
int binfile(FILE *out)
{
    ...
    line_t *curLine = line_head->next;
    long curByte = 0;
    char zeroByte = 0;
    /* binary write y64 code to output file (NOTE: see fwrite()) */
    while(curLine)
    {
        if((curLine->y64bin).bytes != 0)
        {
            // fill 0 until addr
            while(curByte < (curLine->y64bin).addr)
            {
                
                if(fwrite(&zeroByte, 1, 1, out) != 1)
                    return -1;
                curByte++;
            }
            long bytesToWrite = (curLine->y64bin).bytes;
            if(fwrite((curLine->y64bin).codes, 1, bytesToWrite, out) != bytesToWrite)
                return -1;
            curByte += bytesToWrite;
        }
