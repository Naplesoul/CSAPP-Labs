...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    {
    case A_ADD:
        ovf = (argA > 0 && argB > 0 && sign) || (argA < 0 && argB < 0 && !sign);
        break;
    case A_SUB:
        ovf = (argA < 0 && argB > 0 && !sign) || (argA > 0 && argB < 0 && sign);
        break;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
     switch (cond)
    {
        case C_YES:
            return TRUE;
        case C_LE:
            return GET_ZF(cc) || (GET_SF(cc) ^ GET_OF(cc));
        case C_L:
...
stat_t nexti(y64sim_t *sim)
{
    ...
    regid_t regA = REG_ERR, regB = REG_ERR;
    long_t argA, argB, result;
    long_t immediate;
    ...
    // if (0xb < icode || (0x2 == icode && 0x6 < ifun)
    //     || (0x6 == icode && 0x3 < ifun) || (0x7 == icode && 0x6 < ifun))
    //     // || (0x3 == icode && 0xf != regA) || ((0xa == icode || 0xb == icode) && 0xf != regB))
    // {
    //     err_print("PC = 0x%lx, Invalid instruction code", sim->pc);
    //     return STAT_INS;
    // }


    /* get registers if needed (1 byte) */
    if ((0x2 <= icode && 0x6 >= icode) || 0xa == icode || 0xb ==icode)
    {
        if (!get_byte_val(sim->m, next_pc, &codefun))
        {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
    ...
    if ((0x3 <= icode && 0x5>=icode) || 0x7 == icode || 0x8 ==icode)
    {
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        result = compute_alu(A_ADD, immediate, argB);
        if (!set_long_val(sim->m, result, argA))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, result);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        argB = get_reg_val(sim->r, regB);
        result = compute_alu(A_ADD, immediate, argB);
        if (!get_long_val(sim->m, result, &argA))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, result);
            return STAT_ADR;
        }
      ...
      case I_ALU: /* 6:x regA:regB */
        argA = get_reg_val(sim->r, regA);
        argB = get_reg_val(sim->r, regB);
        result = compute_alu(ifun, argB, argA);
        sim->cc = compute_cc(ifun, argB, argA, result);
      ...
      case I_CALL: /* 8:x imm */
        result = get_reg_val(sim->r, REG_RSP) - 8;
        set_reg_val(sim->r, REG_RSP, result);
        if (!set_long_val(sim->m, result, next_pc))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, result);
            return STAT_ADR;
        }
        sim->pc = immediate;
        break;
      ...
      case I_RET: /* 9:0 */
        argA = get_reg_val(sim->r, REG_RSP);
        result = argA + 8;
        if (!get_long_val(sim->m, argA, &argB))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, argA);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, result);
        sim->pc = argB;
      ...
      case I_PUSHQ: /* A:0 regA:F */
        argA = get_reg_val(sim->r, regA);
        argB = get_reg_val(sim->r, REG_RSP);
        result = argB - 8;
        set_reg_val(sim->r, REG_RSP, result);
        if (!set_long_val(sim->m, result, argA))
        {
